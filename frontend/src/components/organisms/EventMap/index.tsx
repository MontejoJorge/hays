...
export const EventMap = ({ events, sources }: EventMapProps) => {
  ...
  useEffect(() => {
    ...
    const resetMapButton = new ResetMapButton(linesRef, leafletMapRef, {
      position: 'topright',
    });
  ...
  useEffect(() => {
    ...
    eventMarkers.forEach((marker) => {
      createEventClickHandler(marker, events, sources, leafletMapRef, linesRef);
    });

    sourceMarkers.forEach((marker) => {
      createSourceClickHandler(marker, events, leafletMapRef, linesRef);
    });
